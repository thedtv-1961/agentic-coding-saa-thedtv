...
export async function updateAward(
  ...
  await assertAdmin();
  if (!input.awardId || !input.categoryId) throw new Error("INVALID_INPUT");
...
export async function deleteAward(awardId: string): Promise<{ ok: boolean }> {
  await assertAdmin();
  if (!awardId) throw new Error("INVALID_INPUT");
